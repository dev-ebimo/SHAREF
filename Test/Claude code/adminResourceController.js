...
async function permanentlyDeleteResource(req, res) {
  try {
    ...
    // Clean up everything that references this resource so we don't leave
    // orphaned rows behind (e.g. a notification pointing at a deleted resource).
    await Promise.all([
      Notification.deleteMany({ resource: resource._id }),
      Bookmark.deleteMany({ resource: resource._id }),
      DownloadLog.deleteMany({ resource: resource._id }),
    ]);
